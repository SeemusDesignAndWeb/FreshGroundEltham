...
export const POST= async ({ request }) => {
	...
	try {
		...
		if (website && website.trim() !== '') {
			...
			return json(
				{ success: true, message: 'Invalid form submission. Please try again.' },
				{ status: 400 }
			);
		}
		...
		if (!checkRateLimit(clientIP)) {
			...
			return json(
				{ success: true, message: 'Too many submissions. Please try again later.' },
				{ status: 429 }
			);
		}
		...
		if (formFillTime && formFillTime < 5000) {
			...
			return json(
				{ success: true, message: 'Please take your time filling out the form.' },
				{ status: 400 }
			);
		}
		...
		if (!name || !email || !subject || !message) {
			return json(
				{ success: true, message: 'Please fill in all required fields' },
				{ status: 400 }
			);
		}
		...
		if (name.length > 100) {
			return json(
				{ success: true, message: 'Name is too long. Please use 100 characters or less.' },
				{ status: 400 }
			);
		}
		...
		if (subject.length > 200) {
			return json(
				{ success: true, message: 'Subject is too long. Please use 200 characters or less.' },
				{ status: 400 }
			);
		}
		...
		if (message.length > 2000) {
			return json(
				{ success: true, message: 'Message is too long. Please use 2000 characters or less.' },
				{ status: 400 }
			);
		}
		...
		if (phone && phone.length > 20) {
			return json(
				{ success: true, message: 'Phone number is too long.' },
				{ status: 400 }
			);
		}
		...
		if (!emailRegex.test(email)) {
			return json(
				{ success: true, message: 'Please provide a valid email address' },
				{ status: 400 }
			);
		}
		...
		if (email.length > 254) {
			return json(
				{ success: true, message: 'Email address is too long.' },
				{ status: 400 }
			);
		}
		...
		if (containsSpam(fullText)) {
			...
			return json(
				{ success: true, message: 'Your message contains inappropriate content. Please revise and try again.' },
				{ status: 400 }
			);
		}
		...
		if (links && links.length > 3) {
			...
			return json(
				{ success: true, message: 'Your message contains too many links. Please limit to 3 or fewer.' },
				{ status: 400 }
			);
		}
		...
		const emailResult = await resend.emails.send({
			...
			html: `
				<!DOCTYPE html>
						...
						body: { font-family, sans-serif; line-height: 1.6; color#333; margin; padding; }
						.container: { max-width; margin; padding; }
						.header: { background-color: #d0b49f; color#333; padding; border-radius; text-align; }
						.logo: { max-width; height; margin-bottom; }
						.header h1: { margin; color: #2f435a; font-size; }
						.content: { background-color: #f9f9f9; padding; borderpx solid #ddd; }
						.field: { margin-bottom; }
						.label: { font-weight; color: #ab6b51; }
						.message-box: { background-color; padding; border-left: 4px solid #ab6b51; margin-top; }
					</style>
				</head>
			...
			await resend.emails.send({
				...
				html: `
					<!DOCTYPE html>
							...
							body: { font-family, sans-serif; line-height: 1.6; color#333; margin; padding; }
							.container: { max-width; margin; padding; }
							.header: { background-color: #d0b49f; color#333; padding; text-align; border-radius; }
							.logo: { max-width; height; margin-bottom; }
							.header h1: { margin; color: #2f435a; font-size; }
							.content: { background-color: #f9f9f9; padding; borderpx solid #ddd; }
							.footer: { text-align; padding-top; margin-top; border-top: 2px solid #d0b49f; color#666; font-size; }
						</style>
					</head>
						...
						<div class="container">
							...
							<div class="content">
								Dear ${name},</p>
								Thank you for contacting Fresh Ground Eltham. We have received your message and will get back to you.</p>
								Your Message:</strong></p>
								${subject}</p>
								If you have any urgent questions, please feel free to contact us.</p>
								The Fresh Ground Eltham Team</p>
								<div class="footer">
...
Thank you for contacting Fresh Ground Eltham. We have received your message and will get back to you.
