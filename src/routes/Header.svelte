<script lang="ts">
	...
	<header class="sticky top-0 z-[100] pointer-events-none transition-all duration-300 w-full">
		...
		<div class="py-2 px-4 border-b w-full transition-colors duration-300 {bannerEnabled && bannerMessage ? 'bg-[#ab6b51] border-[#d0b49f]/30' : 'bg-[#2f435a] border-[#39918c]/30'}">
			<div class="max-w-7xl mx-auto {bannerEnabled && bannerMessage ? 'flex flex-col md:flex-row md:justify-between md:items-center gap-2 md:gap-4' : 'flex justify-between items-center gap-4'}">
				<!-- Banner Message - centered on mobile, right-aligned on desktop -->
				...
				<!-- Social links and cart row - shown on separate row on mobile when banner is enabled -->
				<div class="flex {bannerEnabled && bannerMessage ? 'justify-center md:justify-between' : 'justify-between'} items-center gap-4 pointer-events-auto {bannerEnabled && bannerMessage ? 'order-1 md:order-none w-full md:w-auto' : ''}">
					...
					<div class="flex items-center gap-4 {bannerEnabled && bannerMessage ? 'hidden md:flex' : ''}">
						{#if cartCount > 0}
							<button 
								onclick={() => goto('/cart')}
