		...
		<div class="py-2 px-4 border-b w-full transition-colors duration-300 {bannerEnabled && bannerMessage ? 'bg-[#ab6b51] border-[#d0b49f]/30' : 'bg-[#2f435a] border-[#39918c]/30'}">
			<div class="max-w-7xl mx-auto">
				{#if bannerEnabled && bannerMessage}
					<!-- With Banner: Two rows on mobile, one row on desktop -->
					<div class="flex flex-col md:flex-row md:justify-between md:items-center gap-2 md:gap-4">
						<!-- Social icons row - first on mobile, left on desktop -->
						<div class="flex justify-center md:justify-start items-center gap-4 pointer-events-auto order-1 md:order-none">
				...
				{:else}
					<!-- Without Banner: Social icons left, cart right on desktop; centered on mobile -->
					<div class="flex justify-center md:justify-between items-center gap-4 pointer-events-auto">
